...
| 4. Builtin coverage | runtime ports of `string.*`, `math.*`, `array.*`, `dict.*`, `io.*`, `sys.*`, `file.*`, `buf.*`, `json.*`, `regex.*` | In progress -- all pure `string.*` (except `hash`, whose VM definition folds over unbounded Integers) and `math.*` builtins plus `sys.exit` are ported to the C runtime and translated; `tests/native_builtins.qa` exercises them byte-identically to the VM. Remaining: heap/IO builtins (`string.split`/`join`, `sys.*` fd I/O, `file.*`, `buf.*`, `dict.*`, `json.*`, `regex.*`), ported in the order later stages demand |
